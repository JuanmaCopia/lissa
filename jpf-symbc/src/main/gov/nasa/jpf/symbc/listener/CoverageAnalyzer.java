 ...
 */
package gov.nasa.jpf.symbc.listener;
...
public class CoverageAnalyzer extends ListenerAdapter implements PublisherExtension {
    ...
    static class Coverage {
        ...
        public String toString() {
            ...
            if (this.total <= 0) {
                ...
                s = String.format("%.2f (%d/%d)", ((double) this.covered / this.total), this.covered, this.total);
            }
            return s;
        }
        // =====
    ...
    static class MethodCoverage {
        ...
        boolean getCoveredLines(BitSet executable, BitSet covered) {
            ...
            for (i = inst.length - 1; i >= 0; i--) {
                line = inst[i].getLineNumber();
                if ((!insn.get(i)) && (line > 0)) { // TODO What do we do with instructions that don't have a line
                                                    // number. Is
                                                    // this even possible?
                    covered.clear(line);
                }
    ...
    static class ClassCoverage {
        ...
        boolean isCodeLoaded() {
            ...
            try {
                ...
                log.warning("CoverageAnalyzer problem: " + cie); // Just log the problem but don't fail. We still want
                                                                 // the
    ...
    StringSetMatcher onlyMethods = null;
    ...
    // ADDED
    HashSet<String> methodsToCover = new HashSet<String>();
    ...
    public CoverageAnalyzer(Config conf, JPF jpf) {
        ...
        // ADDED
        String methods = conf.getString("coverage.only.methods");
        String[] sm = methods.split(";");
        for (int i = 0; i < sm.length; i++) {
            methodsToCover.add(sm[i].trim());
        }
        methodName = conf.getString("method");
        String koratClass = conf.getString("symsolve.class");
        String[] cnsplit = koratClass.split("\\.");
        simpleClassName = cnsplit[cnsplit.length - 1];
        techniqueName = conf.getString("technique");
        scope = conf.getString("symbolic.scope", scope);
        // =====
    ...
    private void computeCoverages(String packageFilter, List<Map.Entry<String, ClassCoverage>> clsEntries, Coverage cls,
            Coverage mth, Coverage branch, Coverage block, Coverage line, Coverage insn) {
    ...
    class PublishConsole extends Publish {
        ...
        void printClassCoverages() {
            ...
            computeCoverages("", clsEntries, clsCoverage, mthCoverage, branchCoverage, bbCoverage, lineCoverage,
                    insnCoverage);
            ...
            // ============= ADDED ============= //
            Coverage tbc = new Coverage(0, 0);
            for (Map.Entry<String, ClassCoverage> e : clsEntries) {
                ClassCoverage cc = e.getValue();
                Coverage cov = getTotalBranchCoverage(cc);
                tbc.covered += cov.covered();
                tbc.total += cov.total();
            }
            pw.print("TOTAL BRANCH COVERAGE: ");
            printCoverage(tbc);
            pw.println("");
            pw.println("Covered Methods: " + coveredMethods.toString());

            String fileName = simpleClassName + "-coverage.csv";
            String data = methodName + ", " + techniqueName + ", " + scope + ", " + tbc.toString();

            if (Utils.createFile(fileName))
                Utils.appendToFile(fileName, "Method,Technique,Scope,Coverage");

            Utils.appendToFile(fileName, data);
            // String data = coveredMethods.toString();
            // data = data.substring(1, data.length() - 1);
            // Utils.createFile(fileName);
            // Utils.appendToFile(fileName, simpleClassName + "." + methodName + ", " +
            // data);
            // ================================= //
        }
        ...
        void printMethodCoverages(ClassCoverage cc) {
            ...
            ArrayList<Map.Entry<MethodInfo, MethodCoverage>> mthEntries = Misc.createSortedEntryList(cc.methods,
            ...
            for (Map.Entry<MethodInfo, MethodCoverage> e : mthEntries) {
                ...
                // ADDED
                if (insnCoverage.covered() > 0) {
                    coveredMethods.add(cc.getClassName() + "." + mi.getLongName());
                }
                if (!methodsToCover.contains(cc.getClassName() + "." + mi.getLongName())) {
                    continue;
                }
                // ======
        ...
        // ADDED METHOD
        Coverage getTotalBranchCoverage(ClassCoverage cc) {
            Coverage totalBranchCov = new Coverage(0, 0);

            if (cc.methods == null) {
                return totalBranchCov;
            }
            ...
            for (Map.Entry<MethodInfo, MethodCoverage> e : mthEntries) {
                ...
                // ===============================
                Coverage branchCoverage = mc.getCoveredBranches();
                totalBranchCov.covered += branchCoverage.covered();
                totalBranchCov.total += branchCoverage.total();
            }
